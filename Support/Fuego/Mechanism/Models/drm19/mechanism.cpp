...
namespace thermo
{
    ...
    AMREX_GPU_DEVICE_MANAGED double low_A[84], low_beta[84], low_Ea[84];
    AMREX_GPU_DEVICE_MANAGED double rev_A[84], rev_beta[84], rev_Ea[84];
    AMREX_GPU_DEVICE_MANAGED double troe_a[84],troe_Ts[84], troe_Tss[84], troe_Tsss[84];
    AMREX_GPU_DEVICE_MANAGED double activation_units[84], prefactor_units[84], phase_units[84];
    AMREX_GPU_DEVICE_MANAGED int is_PD[84], troe_len[84], sri_len[84], nTB[84], *TBid[84];
    AMREX_GPU_DEVICE_MANAGED double *TB[84];
...
AMREX_GPU_DEVICE void  productionRate(double * wdot, double * sc, int stride, double T)
{
    ...
    double K_f = prefactor_units[idx] * fwd_A[idx]
    ...
    if (idx < 8) {
        ...
        logPred = log10(redP);
        logFcent = log10(
            (fabs(troe_Tsss[idx]) > 1.e-100 ? (1.-troe_a[idx])*exp(-T/troe_Tsss[idx]) : 0.) 
            + (fabs(troe_Ts[idx]) > 1.e-100 ? troe_a[idx] * exp(-T/troe_Ts[idx]) : 0.) 
            + (troe_len[idx] == 4 ? exp(-troe_Tss[idx] * invT) : 0.) );
    ...
    /* simple three-body correction then */
    if ((idx > 7) && (idx < 14)) {
        double alpha = 0.0;
        for (int i = 0; i < 21; ++i) {
            alpha += sc[stride*i];
