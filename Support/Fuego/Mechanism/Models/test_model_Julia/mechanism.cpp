...
/*Returns the non-QSS stoichiometric coefficients */
...
void comp_Kc(double *  tc, double invT, double *  Kc)
{
    ...
    Kc[0] = g_RT[2] - g_RT[3] - g_RT_qss[0] + g_RT_qss[7];
    Kc[1] = g_RT[1] - g_RT[2] - g_RT_qss[4] + g_RT_qss[7];
    Kc[2] = g_RT[1] - g_RT[3] - g_RT_qss[4] + g_RT_qss[5];
    Kc[3] = -g_RT[1] + g_RT[2] + g_RT_qss[1] - g_RT_qss[6];
    Kc[4] = -g_RT[0] + g_RT[1] + g_RT_qss[1] - g_RT_qss[3];
    Kc[5] = -g_RT[1] + g_RT[2] - g_RT[5] + g_RT_qss[2];
    Kc[6] = g_RT[1] - g_RT[2] - g_RT[3] + g_RT_qss[0];
    Kc[7] = g_RT[1] - 2.000000*g_RT[3] + g_RT_qss[7];
    Kc[8] = -g_RT[1] + g_RT[3] - g_RT[4] + g_RT_qss[6];
    Kc[9] = -g_RT[1] + g_RT[3] - g_RT[5] + g_RT_qss[1];
...
void equilibriumConstants(double *  kc, double *  g_RT, double T)
{
    ...
    /*reaction 1: O + HO2 => OH + O2 */
    kc[0] = exp((g_RT[2] + g_RT_qss[7]) - (g_RT[3] + g_RT_qss[0]));

    /*reaction 2: H + HO2 => O + H2O */
    kc[1] = exp((g_RT[1] + g_RT_qss[7]) - (g_RT[2] + g_RT_qss[4]));

    /*reaction 3: H + H2O2 <=> OH + H2O */
    kc[2] = exp((g_RT[1] + g_RT_qss[5]) - (g_RT[3] + g_RT_qss[4]));

    /*reaction 4: O + CH => H + CO */
    kc[3] = exp((g_RT[2] + g_RT_qss[1]) - (g_RT[1] + g_RT_qss[6]));

    /*reaction 5: H + CH <=> C + H2 */
    kc[4] = exp((g_RT[1] + g_RT_qss[1]) - (g_RT_qss[3] + g_RT[0]));

    /*reaction 6: O + CH2 <=> H + HCO */
    kc[5] = exp((g_RT[2] + g_RT_qss[2]) - (g_RT[1] + g_RT[5]));

    /*reaction 7: H + O2 <=> O + OH */
    kc[6] = exp((g_RT[1] + g_RT_qss[0]) - (g_RT[2] + g_RT[3]));

    /*reaction 8: H + HO2 <=> 2.000000 OH */
    kc[7] = exp((g_RT[1] + g_RT_qss[7]) - (2.000000 * g_RT[3]));

    /*reaction 9: OH + CO <=> H + CO2 */
    kc[8] = exp((g_RT[3] + g_RT_qss[6]) - (g_RT[1] + g_RT[4]));

    /*reaction 10: OH + CH <=> H + HCO */
    kc[9] = exp((g_RT[3] + g_RT_qss[1]) - (g_RT[1] + g_RT[5]));
